...
def update_greater_than_less_than_5(list_of_labels):
    for idx, label in enumerate(list_of_labels):
        greater_than_less_than_5_condition = label>5
        list_of_labels[idx] = np.where(greater_than_less_than_5_condition, 1, 0)
...
def main(config_path, params_path):
    ...
    new_model1 = tf.keras.models.Sequential(base_layer)
    new_model1.add(
        tf.keras.layers.Dense(2, activation="softmax", name="output_layer")
    )

    logging.info(f"{STAGE} model summary: \n{_log_model_summary(new_model1)}")
    ...
    new_model1.save(model_file_path)

    logging.info(f"base model is saved at {model_file_path}")
    logging.info(f"evaluation metrics {new_model1.evaluate(X_test, y_test_bin)}")  # << y_test_bin for our usecase
